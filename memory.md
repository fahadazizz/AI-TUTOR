...
- Phase A V0.8 Done: System underwent internal dogfood validation phase with real-world users. Deep pedagogical fixes executed. Solved the "Goldfish Memory" bug by injecting chat history into the LLM context. Softened harsh scaffolding by introducing a `PROVIDE_WORKED_EXAMPLE` action for struggling students. Handled `CLARIFY_STEP` questions and `EXPRESS_FRUSTRATION` seamlessly without punishing the student's mastery score. Phase A is now a 90/100 success.
- Phase A V0.8 Polish (100/100) Done: Built a robust Chat Persistence system saving history to DB and fetching on React mount, preventing chat amnesia. Softened Prerequisite enforcement by implementing a "Soft Wall" Push/Pull state tracker that warns students but respects their autonomy if they insist. Enforced strict math LaTeX formatting to prevent "MINUS" string generation. Fixed missing `visual_need` diagram tag on Discriminant concept.
- Completed Phase B V1.1 (Pedagogy Router Refactor): Introduced VerifierRegistry, PedagogyPlugin base, QuantitativePlugin, and PedagogyRouter to decouple MathChecker from TutorController. All tests passing and architecture is now extensible for non-quantitative subjects.
